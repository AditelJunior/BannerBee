...
const  Home = () => {
    ...
    async function uploadImage(file) {
        ...
        newImage.src = URL.createObjectURL(file.file);

        return {url: url, name: file.file.name, size: imageSize, description: file.description};
    }

    async function uploadImages(files) {
        const filePromises = Array.from(files, (item) => uploadImage(item));
    ...
    async function aiResponse(request) {
        ...
        for(let i=0;i<fileInputs.length;i++) {
            console.log(fileInputs[i].files[0])
            console.log(fileDesc[i].value)
            filesArr.push({ file: fileInputs[i].files[0], description: fileDesc[i].value})
        }
                                ...
                                return (
                                    <div className="inputs_row" key={'inputRow'+i}>
                                        <div>
                                            ...
                                            <input type="file" id={'fileInput-' + i} accept="image/png, image/jpeg, image/gif"/>
                                        </div>
