...
const  Home = () => {
    ...
    async function aiResponse(request) {
        ...
        if(filesArr && filesArr.length>0) {
            uploadedFiles = await uploadFiles(filesArr);
        }
        let instructionForImages = 'Here are the images i have ';

        console.log(uploadedFiles)

        
        setQuery(document.getElementById('chat_input').value + uploadedFiles.length>0 ? 'Here are the images i have ' : null )
